...
export default function EditRuleModal({
  ...
  const form = useForm<EditRuleFormValues>({
    ...
    defaultValues: {
      ...
      managerApproved: rule?.managerApproved || "",
    },
  });
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent>
        ...
        <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4 pt-2">
          ...
          <div>
            <label className="block mb-1 font-medium">Code Reviewer</label>
            <Input
              {...form.register("managerApproved")}
              placeholder="Reviewer username (optional)"
            />
          </div>
          <DialogFooter>
